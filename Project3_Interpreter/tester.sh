...
echo ""
echo ""
read -n 1 -p "Error is assignment to null object variable. Error message related to that? (y/n)" mainmenuinput
if [ $mainmenuinput = "y" ]; then
	error=$(($error + 1))
fi
echo ""
...
echo ""
echo ""
read -n 1 -p "Error is undefined key value. Error message related to that? (y/n)" mainmenuinput
if [ $mainmenuinput = "y" ]; then
	error=$(($error + 1))
fi
echo ""
